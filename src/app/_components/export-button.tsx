...
export function ExportButton({ deviceId, deviceName, className }: ExportButtonProps) {
  ...
  return (
    <button
      ...
      className={
        className ??
        "flex items-center gap-2 rounded-full border border-mist bg-card px-4 py-2 text-sm font-semibold text-ink transition hover:bg-mist/60 disabled:opacity-50 disabled:cursor-not-allowed"
      }
    >
      {loading ? (
        <>
          <span className="animate-spin inline-block w-4 h-4 border-2 border-mist border-t-sage rounded-full" />
          Exporting…
        </>
      ) : (
        <>
